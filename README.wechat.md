...
- 新手操作与维护指南：[WeChat推送到草稿箱新手操作指南.md](/Users/xiongtao/Documents/zhaop_automotion/WeChat推送到草稿箱新手操作指南.md)
...
按当前仓库的 Docker 默认映射，本文后续所有对外访问示例都统一写成 `http://localhost:5001`。
...
如果你按本文默认的 Docker 暴露方式使用，就访问 `http://localhost:5001/login.html` 扫码登录。
如果你是脚本直跑源码，则改用那个实例实际监听的端口。
...
| 地址 | 说明 |
|------|------|
| http://localhost:5001 | 管理面板 |
| http://localhost:5001/login.html | 扫码登录 |
...
如需通过域名或 HTTPS 访问，按当前 Docker 默认映射可配置 Nginx 反向代理到 `localhost:5001`：
...
医院招聘和 crawler review package 现在可以共用一条“共享草稿串”状态，统一写入同一个公众号草稿箱批次：

- 默认只按 `source_url` 去重
- 当前草稿未满 8 篇时继续追加
- 当前草稿已满 8 篇时自动创建下一份草稿并切到下一批
- 共享状态保存在 `data/rss.db` 的 `official_draft_series` 和 `official_draft_sync`
...
# 强制允许重复推送同一个 source_url
curl -X POST "http://localhost:5001/api/recruitment/official-draft/series/push?source_url=https://example.com/recruit/123&force=true"

# 查看当前共享草稿串状态
curl "http://localhost:5001/api/recruitment/official-draft/series/status"
```

旧接口 `POST /api/recruitment/official-draft/push` 和 `POST /api/recruitment/official-draft/append` 仍然保留，适合手动指定单篇创建或指定 `draft_media_id` 追加。
...
```bash
curl "http://localhost:5001/api/public/searchbiz?query=公众号名称"
```
...
```bash
# 获取前 50 篇
curl "http://localhost:5001/api/public/articles?fakeid=YOUR_FAKEID&begin=0&count=50"

# 获取第 51-100 篇
curl "http://localhost:5001/api/public/articles?fakeid=YOUR_FAKEID&begin=50&count=50"
```
...
```bash
curl "http://localhost:5001/api/public/articles/search?fakeid=YOUR_FAKEID&query=关键词"
```
...
完整的接口文档请访问 http://localhost:5001/api/docs
...
| `SITE_URL` | **网站访问地址（用于RSS图片代理，必须配置）** | http://localhost:5001 |
| `PORT` | 服务端口 | 5000 |
| `HOST` | 监听地址 | 0.0.0.0 |
...
访问 http://localhost:5001/login.html 扫码登录，凭证会自动保存到 `.env`。
</details>
