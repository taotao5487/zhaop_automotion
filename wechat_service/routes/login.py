...
from wechat_service.utils.webhook import webhook
...
async def create_session(sessionid: str, request: Request):
    ...
    **路径参数：**
    ...
    try:
        ...
        if is_https:
            print("🔒 检测到HTTPS环境，Cookie将保留Secure标志（安全传输）")
        else:
            print("⚠️ 检测到HTTP环境，Cookie将移除Secure标志（兼容模式，生产环境建议使用HTTPS）")
