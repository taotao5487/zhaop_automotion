...
from wechat_service.utils.rss_poller import rss_poller, POLL_INTERVAL
from wechat_service.utils.image_proxy import proxy_image_url
...
async def trigger_poll():
    ...
    try:
        ...
        return PollerStatusResponse(
            success=True,
            data={
                "message": "轮询完成" if processed else "当前没有到期订阅",
                "processed": processed,
            }
        )
...
async def export_subscriptions(
    request: Request,
    format: str = Query(
        "csv", pattern="^(csv|opml)$", description="导出格式: csv 或 opml"
    ),
):
    """
    导出当前订阅列表。
...
def _build_aggregated_rss_xml(
    articles: list, nickname_map: dict, base_url: str
) -> str:
