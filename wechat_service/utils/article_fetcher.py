    ...
    try:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                from wechat_service.utils.helpers import has_article_content, is_article_unavailable

                if is_article_unavailable(html):
                    logger.warning(
                        "[Fetch] permanently unavailable (attempt %d/%d) %s",
                        attempt + 1,
                        max_retries + 1,
                        article_url[:60],
                    )
                ...
                logger.warning(
                    "[Fetch] request error: %s (attempt %d/%d)",
                    str(e)[:100],
                    attempt + 1,
                    max_retries + 1,
                )
