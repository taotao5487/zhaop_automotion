...
from shared.paths import ENV_FILE, STATIC_DIR, ensure_runtime_dirs, load_root_env

load_root_env()

# 导入路由
from wechat_service.routes import article, articles, search, admin, login, image, health, stats, rss, recruitment
from wechat_service.utils.official_wx_preview import official_wx_preview_worker
from wechat_service.utils.rss_store import init_db
from wechat_service.utils.rss_poller import rss_poller
...
async def lifespan(app: FastAPI):
    ...
    if not ENV_FILE.exists():
        ...
        print("Visit: http://127.0.0.1:5001/admin.html")
        print("=" * 60 + "\n")
    else:
        print("\n" + "=" * 60)
    ...
    await official_wx_preview_worker.start()
    yield
    await official_wx_preview_worker.stop()
...
app.include_router(recruitment.router, prefix="/api", tags=["医院招聘"])
